// pages/admin.tsx - COMPLETE VERSION WITH ALL 4 SECTIONS
...
export default function Admin() {
  ...
  const [activeTab, setActiveTab] = React.useState<"votes" | "signups" | "analytics" | "track-submissions">("track-submissions");
  ...
  const getAnalytics = () => {
    ...
    const chartData = Object.keys(dailyVisits).sort().slice(-14).map((date) => ({
      date: new Date(date).toLocaleDateString('he-IL', { month: 'short', day: 'numeric' }),
      visits: dailyVisits[date],
    }));

    return { pageVisits, dailyVisits, countryVisits, avgDuration, israelVisits, chartData };
  };
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {signups.map((s) => (
                      <div key={s.id} className="glass rounded-2xl p-4">
                        ...
                        <button onClick={() => setSelectedSignup(s)} className="btn-primary px-3 py-2 rounded-xl text-sm w-full">
                          צפה בפרטים
                        </button>
                      </div>
                    ))}
                    ...
                    {trackSubs.map((track) => (
                      <div key={track.id} className={`glass rounded-2xl p-4 ${track.is_approved ? 'bg-green-900/20 border-2 border-green-500/50' : 'border border-purple-500/30'}`}>
                        ...
                        <div className="flex flex-col gap-2">
                          ...
                          <button onClick={() => setSelectedTrackSub(track)} className="btn-secondary px-3 py-2 rounded-xl text-sm">👁️ צפייה בפרטים</button>
                        </div>
                      </div>
                    ))}
                  </div>
                  ...
                  {analyticsLoading ? (
                    ...
                    <>
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                        ...
                        <div className="glass rounded-2xl p-6">
                          <div className="text-sm text-white/60 mb-1">ממוצע זמן</div>
                          <div className="text-3xl font-bold text-purple-400">{formatDuration(Math.round(analytics.avgDuration))}</div>
                        </div>
                        <div className="glass rounded-2xl p-6">
                          <div className="text-sm text-white/60 mb-1">מישראל</div>
                          <div className="text-3xl font-bold text-green-400">{analytics.israelVisits}</div>
                        </div>
                        <div className="glass rounded-2xl p-6">
                          <div className="text-sm text-white/60 mb-1">מדינות</div>
                          <div className="text-3xl font-bold text-yellow-400">{Object.keys(analytics.countryVisits).length}</div>
                        </div>
                      </div>
                      <div className="glass rounded-2xl p-6">
                        <h3 className="text-xl font-semibold mb-4">ביקורים יומיים</h3>
