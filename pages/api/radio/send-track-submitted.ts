// pages/api/radio/send-track-submitted.ts
import type { NextApiRequest, NextApiResponse } from 'next';
import { resend, EMAIL_SENDER } from '@/lib/resend';

const createTrackSubmittedEmailHTML = (artistName: string, trackName: string) => `
<!DOCTYPE html>
          ...
          <tr>
            <td style="background: rgba(255,255,255,0.03); padding: 40px; border: 1px solid rgba(255,255,255,0.1); border-top: none; border-bottom: none;">
              ...
              <p style="color: #e2e8f0; font-size: 16px; line-height: 1.8; margin: 0 0 24px; text-align: right;">
                תודה על ההגשה! הטראק שלך נכנס לתור והצוות שלנו יאזין לו בהקדם.
              </p>
              ...
              <p style="color: #94a3b8; font-size: 14px; line-height: 1.7; margin: 0 0 32px; text-align: right;">
                בינתיים, אפשר לעקוב אחרי הסטטוס בלוח הבקרה שלך.
              </p>
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const { email, artistName, trackName } = req.body;

  // Validate inputs
  if (!email || !artistName || !trackName) {
    console.error('❌ Missing required fields:', { email: !!email, artistName: !!artistName, trackName: !!trackName });
  ...
  console.log('📧 Sending track submitted email to:', email);

  try {
    const data = await resend.emails.send({
      from: EMAIL_SENDER,
      to: [email],
      subject: `📤 הטראק "${trackName}" התקבל בהצלחה!`,
      html: createTrackSubmittedEmailHTML(artistName, trackName),
    });
...
resend.emails.send({
  ...
  subject: `🎵 טראק חדש הוגש: ${trackName}`,
  html: `<div style="font-family: sans-serif; direction: rtl; padding: 20px;">
    <h2>טראק חדש הוגש לרדיו!</h2>
    <p><strong>שם הטראק:</strong> ${trackName}</p>
    <p><strong>אמן:</strong> ${artistName}</p>
    <p><strong>אימייל:</strong> ${email}</p>
    <p><a href="https://tracktrip.co.il/admin/radio">לצפייה ואישור בפאנל הניהול</a></p>
  </div>`,
