// pages/api/track-submissions.ts (REWRITE: Supports GET and POST/DELETE)
import type { NextApiRequest, NextApiResponse } from 'next';
import { createClient } from '@supabase/supabase-js';

const SUBMISSION_TABLE = 'track_of_the_week_submissions';
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const supabaseUrl = process.env.SUPABASE_URL; // Using SUPABASE_URL as confirmed working
  ...
  try {
    ...
    if (req.method === 'POST' && req.body?.action === 'delete') {
      // Delete a specific track submission
