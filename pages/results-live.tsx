...
export default function ResultsLive() {
  ...
  async function fetchPublicStats() {
    try {
      ...
      if (j?.ok) {
        setTally(j.tally);
        // Don't use j.totalVotes - we'll get it from vote-count API
  ...
  // Fetch the "fake" inflated vote count (same as landing page)
  async function fetchVoteCount() {
    try {
      const r = await fetch(`/api/vote-count?_t=${Date.now()}`);
      const j = await r.json();
      
      if (j?.ok && typeof j.count === "number") {
        setTotalVotes(j.count);
      }
    } catch (err) {
