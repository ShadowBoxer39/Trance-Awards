...
export default function DuelsAdmin() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.ok) {
        alert(`הדו-קרב נוסף בהצלחה!\nתאריך פרסום: ${new Date(data.duel.publish_date).toLocaleDateString('he-IL')}`);
        fetchDuels(key);
        setFormData({
            type: 'track',
      ...
      <div className="max-w-6xl mx-auto">
        <div className="flex justify-between items-center mb-8">
            ...
            <div className="flex items-center gap-4">
                <div className="text-sm bg-purple-900/50 px-3 py-1 rounded-full border border-purple-500/30">
                    ℹ️ התור נקבע אוטומטית (כל 3 ימים)
                </div>
        ...
        <div className="grid lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-1">
                <form onSubmit={handleSubmit} className="glass-card p-6 rounded-2xl sticky top-6">
                    <h2 className="text-xl font-bold mb-4 text-cyan-400">הוסף לתור</h2>
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="p-3 border border-red-500/30 rounded-xl bg-red-500/5">
                            ...
                            {formData.type === 'track' && (
                                <input 
                                    placeholder="לינק ליוטיוב (חובה לטראק)" 
                                    className="w-full bg-black/40 border border-white/10 rounded p-2 mb-2 text-sm"
                            ...
                            <input 
                                placeholder={formData.type === 'track' ? "תמונה (ריק = אוטומטי מיוטיוב)" : "לינק לתמונה (חובה)"}
                                className="w-full bg-black/40 border border-white/10 rounded p-2 text-sm"
            ...
            <div className="lg:col-span-2 space-y-4">
                <h2 className="text-xl font-bold text-white/80">התור הנוכחי ({duels.length})</h2>
                {duels.map((duel, index) => {
                    const date = new Date(duel.publish_date);
                    const isFuture = date > new Date();
                    const isToday = new Date().toDateString() === date.toDateString();
                    
                    return (
                        <div key={duel.id} className={`glass-card p-4 rounded-xl flex gap-4 items-center ${isToday ? 'border-2 border-green-500 bg-green-500/10' : 'opacity-80'}`}>
                            <div className="text-center min-w-[90px]">
                                <div className="text-xs text-gray-400">{date.toLocaleDateString('he-IL')}</div>
                                <div className="text-lg font-bold">{date.toLocaleTimeString('he-IL', {hour: '2-digit', minute:'2-digit'})}</div>
                                {isToday && <span className="text-[10px] bg-green-500 text-black px-2 rounded-full font-bold">פעיל עכשיו</span>}
                                {isFuture && <span className="text-[10px] bg-blue-500 text-black px-2 rounded-full">בתור</span>}
                            </div>

                            <div className="flex-1 grid grid-cols-2 gap-4">
                                <div className="flex items-center gap-2">
                                    {duel.image_a ? (
                                       <img src={duel.image_a} className="w-12 h-12 rounded object-cover" />
                                    ) : (
                                ...
                                <div className="flex items-center gap-2">
                                    {duel.image_b ? (
                                       <img src={duel.image_b} className="w-12 h-12 rounded object-cover" />
                                    ) : (
                                       <div className="w-12 h-12 bg-gray-700 rounded flex items-center justify-center">?</div>
                                    )}
                                    <div className="overflow-hidden">
