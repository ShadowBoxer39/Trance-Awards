...
export default function QuizWidget() {
  ...
  const [useAudioPlayer, setUseAudioPlayer] = useState(false); // <--- NEW: Controls Fallback
  ...
  const fetchQuiz = async () => {
    try {
      ...
      if (data.ok) {
        ...
        // Initialize audio player preference
  ...
  const initYouTubePlayer = () => {
    // Only load YouTube if audio player is disabled/failed OR we have no audio URL
  ...
  useEffect(() => {
    // Decide which player to init based on useAudioPlayer state
    if (useAudioPlayer && quiz?.audioUrl) {
       setIsPlaying(false);
       setProgress(0);
  ...
  const handleAudioError = (e: any) => {
    console.error("Audio Stream Failed - Switching to YouTube Fallback");
    // CRITICAL FIX: Disable audio player to force YouTube init
  ...
  const playSnippet = () => {
    ...
    if (useAudioPlayer && quiz.audioUrl && audioPlayerRef.current) {
        // Ensure we are at the start time
