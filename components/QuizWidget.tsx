...
export default function QuizWidget() {
  ...
  // --- AUDIO EVENT HANDLERS (The Fix for "No Sound") ---
  
  const handleAudioMetadata = () => {
    // When audio loads, seek to the start time immediately
  ...
  const handleAudioError = (e: any) => {
    ...
    setIsPlaying(false);
    alert("Audio stream failed. Please refresh.");
  };
  ...
  const playSnippet = () => {
    ...
    if (quiz.audioUrl && audioPlayerRef.current) {
        // Seek to start time again just to be safe
