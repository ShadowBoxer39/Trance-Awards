// components/QuizWidget.tsx - UPDATED with robust secure audio player
...
interface QuizData {
  ...
  audioUrl: string | null;  // This is now a SECURE proxy URL
...
export default function QuizWidget() {
  ...
  // Player State - SIMPLIFIED for reliability
  ...
  // ═══════════════════════════════════════════════════════════════════
  // 
  // The audioUrl is a PROXY URL like /api/quiz/stream?id=ENCRYPTED
  // The user CANNOT see the real video ID or track name!
  // 
  // ═══════════════════════════════════════════════════════════════════

  const playSnippet = async () => {
    if (!quiz || !quiz.audioUrl) {
      setPlayerError("אין קובץ אודיו זמין");
      return;
    }
    ...
    try {
      ...
      if (!audioRef.current) {
        ...
        audioRef.current.addEventListener('error', (e) => {
          ...
          setPlayerError("שגיאה בטעינת האודיו");
          setIsPlaying(false);
          setIsBuffering(false);
        });
      ...
      audioRef.current.src = quiz.audioUrl;
      ...
      setPlayerError(error.message || "שגיאה בהפעלת האודיו");
      setIsPlaying(false);
      setIsBuffering(false);
    }
  };
  ...
  const stopPlayback = () => {
    ...
    if (audioRef.current) {
      ...
      audioRef.current.currentTime = quiz?.youtubeStart || 0;
    }
  };
  ...
  // Success state (answered correctly)
  if (attempts?.hasCorrectAnswer || result?.isCorrect || scoreSaved) {
    return (
          ...
          <div className="mb-8">
            ...
            <p className="text-gray-300 mb-4">
              {scoreSaved 
                ...
                : `ענית נכון ב-${attempts?.used || 1} ניסיונות`}
            </p>
            {result && !scoreSaved && (
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex flex-col">
                        <span className={`text-sm font-medium ${
                          entry.userId === user?.id ? "text-purple-300" : "text-gray-200"
                        }`}>
                          {entry.displayName}
                        </span>
        ...
        <div className="text-center">
          ...
          <div className="bg-white/5 rounded-xl p-4 border border-white/10">
            <p className="text-sm text-gray-400">
              החידון הבא: <span className="text-cyan-400 font-medium">
  ...
  return (
    <div id="quiz-widget-section" className="glass-card rounded-xl overflow-hidden border-2 border-cyan-500/30">
      ...
      <div className="p-6 md:p-8">
        ...
        {/* AUDIO PLAYER - Secure, no track name exposed */}
        {quiz.type === "snippet" && quiz.audioUrl && (
          <div className="mb-6">
        ...
        <div className="space-y-4">
          ...
          <button 
            ...
            disabled={submitting || (quiz.type === "snippet" ? !artistAnswer || !trackAnswer : !triviaAnswer)} 
