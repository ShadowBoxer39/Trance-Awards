...
// ... inside QuizWidget ...
 ...
 const playSnippet = () => {
  ...
  const submitAnswer = async () => {
    ...
    } catch (error) { alert("שגיאה בשליחה"); } 
  ...
  const saveScore = async () => {
    ...
    try {
      const res = await fetch("/api/quiz/save-score", {
        ...
        body: JSON.stringify({ questionId: quiz.id, odau: user.id, displayName: userName, photoUrl: userPhoto, isArchive: false }),
      });
      const data = await res.json();
      if (data.ok || data.error === "score_already_saved") {
        setScoreSaved(true);
        fetchLeaderboard();
      }
        ...
        <div className="text-center">
          ...
          <div className="bg-gradient-to-r from-cyan-500/10 to-purple-500/10 rounded-xl p-4 border border-white/10 mb-6">
            <p className="text-gray-300">
              {nextQuizDay === "Monday" ? "🎵 החידון הבא יום שני - נחשו את הטראק!" : nextQuizDay === "Thursday" ? "🧠 החידון הבא יום חמישי - טריוויה!" : "החידון הבא בקרוב!"}
            </p>
          </div>
          {previousAnswer && (
            <div className="bg-black/30 rounded-xl p-4 border border-white/10 text-right">
              ...
              <p className="text-lg font-semibold text-cyan-400">
                {typeof previousAnswer.answer === "string" ? previousAnswer.answer : `${previousAnswer.answer.artist} - ${previousAnswer.answer.track}`}
              </p>
            </div>
          )}
        </div>
        ...
        <div className="text-center">
          <div className="mb-8">
            ...
            <p className="text-gray-300 mb-4">
              {scoreSaved ? "הניקוד שלך שמור במערכת!" : `ענית נכון ב-${attempts?.used || (result?.pointsEarned === 3 ? 1 : result?.pointsEarned === 2 ? 2 : 3)} ניסיונות`}
            </p>
            {result && !scoreSaved && (
              <div className="inline-block bg-cyan-500/20 border border-cyan-500/30 rounded-full px-4 py-1">
          ...
          {!scoreSaved && (
            <div className="bg-black/30 border border-white/10 rounded-xl p-6 mb-8">
              ...
              <p className="text-sm text-gray-400 mb-4">התחבר כדי להופיע בטבלת המובילים ולהתחרות עם חברים!</p>
              ...
              {isProcessingAuth ? (
                ...
                <button onClick={saveScore} className="w-full bg-gradient-to-r from-purple-500 to-cyan-500 hover:from-purple-600 hover:to-cyan-600 text-white font-bold py-3 px-6 rounded-lg transition-all shadow-lg hover:shadow-purple-500/25">
                  💾 שמור ללידרבורד
                </button>
              ) : (
          ...
          <div className="text-right border-t border-white/10 pt-6">
            <h4 className="text-lg font-bold text-white mb-4 flex items-center gap-2"><span>🏆</span> טבלת המובילים</h4>
            {loadingLeaderboard ? <div className="text-center py-4 text-gray-500">טוען טבלה...</div> 
            : leaderboard.length === 0 ? <div className="text-center py-4 text-gray-500">אין עדיין מובילים. הייה הראשון!</div> 
            : (
              <div className="space-y-2 max-h-80 overflow-y-auto pr-1 custom-scrollbar">
                {leaderboard.map((entry, idx) => (
                  <div key={entry.userId} className={`flex items-center justify-between p-3 rounded-lg transition-colors ${entry.userId === user?.id ? "bg-purple-500/20 border border-purple-500/50" : "bg-white/5 hover:bg-white/10"}`}>
                    <div className="flex items-center gap-3">
                      <div className={`w-6 h-6 flex items-center justify-center rounded-full text-xs font-bold ${idx === 0 ? "bg-yellow-500 text-black" : idx === 1 ? "bg-gray-300 text-black" : idx === 2 ? "bg-orange-400 text-black" : "bg-white/10 text-gray-400"}`}>{idx + 1}</div>
                      {entry.photoUrl ? <img src={entry.photoUrl} alt={entry.displayName} className="w-8 h-8 rounded-full object-cover border border-white/20"/> : <div className="w-8 h-8 rounded-full bg-white/10 flex items-center justify-center text-sm">👤</div>}
                      <div className="flex flex-col">
                        <span className={`text-sm font-medium ${entry.userId === user?.id ? "text-purple-300" : "text-gray-200"}`}>{entry.displayName}</span>
                        <span className="text-[10px] text-gray-500">ענה על {entry.questionsAnswered} שאלות</span>
                      </div>
                    </div>
                    <div className="font-bold text-cyan-400 tabular-nums">{entry.totalPoints} נק׳</div>
                  </div>
                ))}
              </div>
            )}
      ...
      <div id="quiz-widget-section" className="glass-card rounded-xl p-8 border-2 border-red-500/30 bg-gradient-to-b from-red-500/5 to-transparent">
        <div className="text-center">
          ...
          <div className="bg-white/5 rounded-xl p-4 border border-white/10">
            <p className="text-sm text-gray-400">החידון הבא: <span className="text-cyan-400 font-medium">{new Date().getDay() < 4 ? "יום חמישי (טריוויה)" : "יום שני (נחשו את הטראק)"}</span></p>
          </div>
        </div>
      </div>
    );
      ...
      <div className={`p-4 ${quiz.type === "snippet" ? "bg-gradient-to-r from-cyan-500/20 to-purple-500/20" : "bg-gradient-to-r from-purple-500/20 to-pink-500/20"}`}>
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl md:text-2xl font-bold text-white">{quiz.type === "snippet" ? "נחשו את הטראק" : "טריוויית טראנס"}</h2>
              <p className="text-sm text-white/60">{quiz.type === "snippet" ? "הקשיבו לקטע ונחשו את השיר" : "ענו על השאלה"}</p>
            </div>
          </div>
          <div className="flex items-center gap-2 bg-black/30 rounded-full px-4 py-2">
            ...
            <div className="flex gap-1">{[1, 2, 3].map((n) => (<div key={n} className={`w-3 h-3 rounded-full transition-all ${n <= (attempts?.used || 0) ? "bg-gray-600" : "bg-cyan-400 shadow-lg shadow-cyan-400/50"}`}/>))}</div>
          </div>
        </div>
      </div>
      ...
      <div className="p-6 md:p-8">
        {quiz.contributor && (
          <div className="flex items-center gap-3 mb-6 p-3 bg-white/5 rounded-lg border border-white/10">
            {quiz.contributor.photo_url ? <img src={quiz.contributor.photo_url} alt={quiz.contributor.name} className="w-10 h-10 rounded-full object-cover"/> : <div className="w-10 h-10 rounded-full bg-purple-500/30 flex items-center justify-center"><span className="text-lg">👤</span></div>}
            <div><p className="text-xs text-gray-400">שאלה מאת</p><p className="text-sm font-medium text-purple-400">{quiz.contributor.name}</p></div>
          </div>
        )}
        ...
        <div className="space-y-4">
          {quiz.type === "snippet" ? (
            <>
              <div>
                ...
                <input type="text" value={artistAnswer} onChange={(e) => setArtistAnswer(e.target.value)} className="w-full bg-black/50 border-2 border-white/10 rounded-xl px-4 py-3 text-white placeholder-gray-500 focus:border-cyan-500 focus:outline-none transition-all" placeholder="הקלידו את שם האמן..." disabled={submitting}/>
              </div>
              <div>
                <label className="block text-sm text-gray-400 mb-2 font-medium">שם הטראק</label>
                <input type="text" value={trackAnswer} onChange={(e) => setTrackAnswer(e.target.value)} className="w-full bg-black/50 border-2 border-white/10 rounded-xl px-4 py-3 text-white placeholder-gray-500 focus:border-cyan-500 focus:outline-none transition-all" placeholder="הקלידו את שם הטראק..." disabled={submitting}/>
              </div>
            </>
          ) : (
            <div>
              <label className="block text-sm text-gray-400 mb-2 font-medium">התשובה שלך</label>
              <input type="text" value={triviaAnswer} onChange={(e) => setTriviaAnswer(e.target.value)} className="w-full bg-black/50 border-2 border-white/10 rounded-xl px-4 py-3 text-white placeholder-gray-500 focus:border-purple-500 focus:outline-none transition-all" placeholder="הקלידו את התשובה..." disabled={submitting}/>
            </div>
          )}
          ...
          <button onClick={submitAnswer} disabled={submitting || (quiz.type === "snippet" ? !artistAnswer || !trackAnswer : !triviaAnswer)} className="w-full bg-gradient-to-r from-cyan-500 to-purple-500 hover:from-cyan-600 hover:to-purple-600 disabled:from-gray-600 disabled:to-gray-600 text-white py-4 rounded-xl font-bold disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-lg hover:shadow-cyan-500/25 hover:scale-[1.01]">
            {submitting ? <span className="flex items-center justify-center gap-2"><div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" />בודק...</span> : "שלח תשובה"}
          </button>
        </div>

        <div className="mt-6 pt-4 border-t border-white/10 text-center">
          <p className="text-sm text-gray-500"><span className="text-cyan-400">ניסיון ראשון</span> = 3 נקודות • <span className="text-purple-400"> שני</span> = 2 נקודות • <span className="text-pink-400"> שלישי</span> = 1 נקודה</p>
        </div>
      </div>
    </div>
  );
